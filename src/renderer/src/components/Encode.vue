...
const convertToWebp = (arr: ImageObj[]) => {
  ...
  infos.forEach((info) => {
    ...
    const parmas = new EncodeParameters(
      isAuto.value,
      isLossless.value,
      zCompression.value,
      quality.value,
...
const queuedConvertToWebp = async (arr: ImageObj[]) => {
  ...
  for (const info of infos) {
    const name = info.name.split('.').slice(0, -1).join('.')
    const outputPath = './output/' + name + '.webp'
    const params = new EncodeParameters(
      isAuto.value,
      isLossless.value,
      zCompression.value,
      quality.value,
...
<template>
  <div id="encode-related">
    <p v-if="currentlyEncoding && !encodeFinished">
      Encoding...{{ encodingProgress }}/{{ selectedImages.length }}
    </p>
    <p v-else-if="encodeFinished">Finished!</p>
    <progress
      v-if:="currentlyEncoding"
  ...
  <div class="container" id="workspace">
    ...
    <div>
      <div class="container vertical sticky-10" id="tweaker">
        <div class="borderless-container" id="dropzone" :class="dragzone">
          ...
          />
          <label for="file-input" @dragover="dragover" @dragleave="dragleave" @drop="filesDropped"
            >Drop files here or click on me</label
          >
        </div>
        <div id="button-list">
          <button class="small-button" @click="openFolder">Open folder</button>
          <button class="small-button" @click="queuedConvertToWebp(selectedImages)">
            Encode selected
          </button>
          <button class="small-button" @click="clearImagesPanel">Clear All</button>
        </div>
        <div id="button-list">
          <button v-if="imageObjs.length !== 0" class="small-button" @click="selectAll">
            Select all
          </button>
    ...
    <div class="container" id="images-viewer">
      <img
        v-for="image in imageObjs"
        :src="image.fileURL"
        @click.ctrl="addToselectedImages"
        @click.exact="loadImageToDetailedViewer"
        :style="isImageSelected(image.fileURL) ? selectedBorder : ''"
      />
    </div>
